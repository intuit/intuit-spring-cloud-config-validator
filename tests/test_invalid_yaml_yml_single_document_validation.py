...
FIXTURE_DIR = FIXTURES_DIR_PATH + "/invalid-yaml-configs-single-documents"

class InvalidYamlSingleDocumentFileTests(unittest.TestCase, ValidationAssertions):
  ...
  def test_some_yaml_yml_files_are_invalid(self):
    ...
    for filePath, validationObject in self.validationIndex.items():
      ...
      if "publisher-onboard_preprod" in filePath or "publisher-dev" in filePath:
